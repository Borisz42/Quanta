...
def test_packing_and_unpacking_roundtrip():
    # Generate random 256 quaternary array
    rng = np.random.default_rng(123)
    orig_arr = rng.integers(0, 4, size=256, dtype=np.uint8)

    packed_bytes = pack_quaternary_array(orig_arr)
    assert len(packed_bytes) == 64

    unpacked_arr = unpack_quaternary_bytes(packed_bytes)
    assert len(unpacked_arr) == 256
...
def test_quanta_vector_initialization_and_access():
    ...
    assert len(vec) == 256
    assert all(vec[i] == QuaternaryValue.IRRELEVANT for i in range(256))
...
def test_hamming_distance_and_similarity():
    ...
    assert v1.similarity(v2) == 1.0 - (2.0 / 256.0)
...
def test_canonical_slots_integrity():
    ...
    for i in range(256):
        ...
        if 0 <= i < 64:
            assert slot.band == SlotBand.BAND_0_NSM_KINEMATICS
        elif 64 <= i < 128:
            assert slot.band == SlotBand.BAND_1_VALENCIES_TOPOLOGY
        elif 128 <= i < 192:
            assert slot.band == SlotBand.BAND_2_ONTOLOGY_MODALITY
        else:
            assert slot.band == SlotBand.BAND_3_EPISTEMIC_METARULES
