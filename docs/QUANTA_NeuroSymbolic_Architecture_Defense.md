...
* **Band 3 (384–511)**: 128 ConceptNet Ontological Taxonomies & Scientific Domains (`CN_Q001_COMPUTING` $\dots$ `CN_Q128_MANNER`) derived via Usage-Weighted Ontological Density Scoring (U-ODS) and Multi-Way Inverted-Index partition refinement across 34M ConceptNet assertions in Belnap 4-valued logic ($\mathcal{B}_4$).
* **Band 4 (512–639)**: 128 ConceptNet Cyber-Physical Tool Affordances, Mechanical Actions & Functional Capabilities (`CN_Q129_LEAVE` $\dots$ `CN_Q256_WORTHY`), enabling 2-tier vector decoding (25,292 singletons in $<5\text{ ms}$ SIMD via $4 \times 4$ cost matrix + SQLite category basin fallback).
