...
def generate_all_examples(output_dir: Path):
    ...
    # ------------------------------------------------------------------
    # 2. Combined Master Graph Markdown File
    # ------------------------------------------------------------------
    with open(output_dir / "all_translation_graphs.md", "w", encoding="utf-8") as f:
        ...
        f.write("This document compiles the complete Abstract Syntax Graph (ASG) node hierarchies, relation edges, active quaternary semantic slots, and Mermaid diagrams for English round-trip benchmark examples.\n\n")
        f.write("---\n\n")
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# QUANTA Two-Way Translation & Benchmarks\n\n")
        f.write("This document presents authentic input-output translation pairs generated via the Content-Addressed Neuro-Symbolic **QUANTA ASG** representation over $\\Sigma^{1024}$.\n\n")
        ...
        f.write("\n---\n\n")
        f.write("### Detailed Sample Trace: English $\\to$ Mentalese ASG $\\to$ English\n\n")
        sample = eng_eng_records[1]  # Dog bit mailman in garden
        f.write(f"**Input:** `{sample['input_text']}`  \n")
        f.write(f"**Reconstructed Output:** `{sample['output_text']}`  \n")
